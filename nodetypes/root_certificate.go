...
func (r *RootCertificate) Store(ctx context.Context, storage nodee.Storage, opt ...nodee.Option) error {
	const op = "nodee.nodetypes.(RootCertificate).Store"
...
//
// Supported options: WithWrapper
func LoadRootCertificate(ctx context.Context, storage nodee.Storage, id string, opt ...nodee.Option) (*RootCertificate, error) {
	const op = "nodee.nodetypes.LoadRootCertificate"
...
// nodee.CurrentId and nodee.NextId
func LoadRootCertificates(ctx context.Context, storage nodee.Storage, opt ...nodee.Option) (*RootCertificates, error) {
	const op = "nodee.nodetypes.LoadRootCertificates"
	ret := new(RootCertificates)
	for _, id := range []string{nodee.CurrentId, nodee.NextId} {
		root, err := LoadRootCertificate(ctx, storage, id, opt...)
		if err != nil {
			return nil, fmt.Errorf("(%s) error loading root certificate: %w", op, err)
		}
		switch id {
		case nodee.CurrentId:
			ret.Current = root
		default:
			ret.Next = root
		}
...
func (r *RootCertificate) SigningParams(ctx context.Context) (*x509.Certificate, crypto.Signer, error) {
	const op = "nodee.nodetypes.(RootCertificate).SigningParams"
	switch {
